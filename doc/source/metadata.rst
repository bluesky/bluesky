...
custom metadata fields 'sample_id', 'purpose', and 'operator'.
...
To be more selective, pass the metadata into the plan instead. Every
...
plan. Thus, with this call

.. code-block:: python

    RE(plan(), sample_id='A', operator='Dan')

each run's metadata will include sample_id, operator, and purpose.
...
.. note::
    ...
    plans are not technically required to provide any of this, but it is a nice
    pattern to follow.
