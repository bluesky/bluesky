...
.. note::
    ...
    something in that category, you're in luck: your problem is simple! Read
    about multidimensional plans in the section on :doc:`plans`. You don't need
...
the data acquisition rates are faster than the RunEngine or Python can go. As a
point of reference, the RunEngine processes a message in one second plus any
compute time required by the message itself.
...
2. Complete: This step tells the device, "I am ready whenver you are ready." If
   the device is just collecting until it is told to stop, it will report that
   is it ready immediately. If the device is executing some predetermined
   trajectory, it will finish before reporting ready.
3. Collect: Finally, the data accumulated by the device is transferred to the
   RunEngine and processed like any other data.
...
To monitor some ``signal`` during a plan, bluesky provides a preprocessor. It
