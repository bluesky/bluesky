...
def test_truncate_json_overflow():
    ...
    assert orjson.dumps(truncated_data)
    for val in orjson.loads(orjson.dumps(truncated_data)).values():
    ...
    data = {"nan": float("nan")}
    truncated_data = truncate_json_overflow(data)
    assert orjson.loads(orjson.dumps(truncated_data))["nan"] is None
