...
def approx_multi_traj_checker(RE, scan, expected_data, *, decimal=2):
    ...
    actual_values = [[row[key] for key in keys] for row in actual_data]

    expected_values = [[row[key] for key in keys] for row in expected_data]

    err_msg = "Trajectory differs. Data keys: {}".format(", ".join(keys))
    numpy.testing.assert_almost_equal(actual_values, expected_values, decimal=decimal, err_msg=err_msg)


def test_outer_product_ascan(RE, hw):
    scan = bp.grid_scan([hw.det], hw.motor1, 1, 3, 3, hw.motor2, 10, 20, 2, False)
...
def test_outer_product_ascan_snaked(RE, hw):
    scan = bp.grid_scan([hw.det], hw.motor1, 1, 3, 3, hw.motor2, 10, 20, 2, True)
    # Note: motor1 is the first motor specified, and so it is the "slow"
    # axis, matching the numpy convention.
    expected_data = [
        {"motor2": 10.0, "det": 1.0, "motor1": 1.0},
...
def test_inner_product_ascan(RE, hw):
    scan = bp.inner_product_scan([hw.det], 3, hw.motor1, 1, 3, hw.motor2, 10, 30)
    # Note: motor1 is the first motor specified, and so it is the "slow"
    # axis, matching the numpy convention.
    expected_data = [
        {"motor2": 10.0, "det": 1.0, "motor1": 1.0},
...
def test_outer_product_dscan(RE, hw):
    scan = bp.rel_grid_scan([hw.det], hw.motor1, 1, 3, 3, hw.motor2, 10, 20, 2, False)
    # Note: motor1 is the first motor specified, and so it is the "slow"
    # axis, matching the numpy convention.
    hw.motor1.set(5)
    hw.motor2.set(8)
...
def test_outer_product_dscan_snaked(RE, hw):
    scan = bp.rel_grid_scan([hw.det], hw.motor1, 1, 3, 3, hw.motor2, 10, 20, 2, True)
    # Note: motor1 is the first motor specified, and so it is the "slow"
    # axis, matching the numpy convention.
    hw.motor1.set(5)
    hw.motor2.set(8)
...
def test_inner_product_dscan(RE, hw):
    scan = bp.relative_inner_product_scan([hw.det], 3, hw.motor1, 1, 3, hw.motor2, 10, 30)
    # Note: motor1 is the first motor specified, and so it is the "slow"
    # axis, matching the numpy convention.
    hw.motor1.set(5)
    hw.motor2.set(8)
...
def test_multi_motor_list_scan(RE, hw):
    expected_data = [
        {"motor2": 18, "motor2_setpoint": 18, "det": 1, "motor1": 6, "motor1_setpoint": 6},
        {"motor2": 28, "motor2_setpoint": 28, "det": 1, "motor1": 7, "motor1_setpoint": 7},
        {"motor2": 38, "motor2_setpoint": 38, "det": 1, "motor1": 8, "motor1_setpoint": 8},
    ]

    scan = bp.list_scan([hw.det], hw.motor1, [6, 7, 8], hw.motor2, [18, 28, 38])
    multi_traj_checker(RE, scan, expected_data)
...
def test_multi_motor_rel_list_scan(RE, hw):
    ...
    hw.motor1.set(5)
    hw.motor2.set(8)
    scan = bp.rel_list_scan([hw.det], hw.motor1, np.array([1, 2, 3]), hw.motor2, np.array([10, 20, 30]))
    multi_traj_checker(RE, scan, expected_data)
...
def test_multi_motor_rel_list_scan_list_input(RE, hw):
    ...
    hw.motor1.set(5)
    hw.motor2.set(8)
    scan = bp.rel_list_scan([hw.det], hw.motor1, [1, 2, 3], hw.motor2, [10, 20, 30])
    multi_traj_checker(RE, scan, expected_data)
...
def test_list_grid_scan(RE, hw):
    ...
    scan = bp.list_grid_scan([hw.det], hw.motor1, [6, 7, 8], hw.motor2, [18, 28, 38])
    multi_traj_checker(RE, scan, expected_data)
...
def test_rel_list_grid_scan(RE, hw):
    ...
    hw.motor1.set(5)
    hw.motor2.set(8)
    scan = bp.rel_list_grid_scan([hw.det], hw.motor1, [1, 2, 3], hw.motor2, [10, 20, 30])
    multi_traj_checker(RE, scan, expected_data)
...
def test_adaptive_dscan(RE, hw):
    ...
    scan3 = bp.rel_adaptive_scan([hw.det], "det", hw.motor, 0, 5, 0.1, 1, 0.1, False)
    scan4 = bp.rel_adaptive_scan([hw.det], "det", hw.motor, 5, 0, 0.1, 1, 0.1, False)
    ...
    with pytest.raises(ValueError):  # min step > max step
        scan5 = bp.rel_adaptive_scan([hw.det], "det", hw.motor, 5, 0, 1, 0.1, 0.1, False)
        RE(scan5)
...
def test_count(RE, hw):
    ...
    RE(plan, {"event": col})
    assert actual_intensity == [1.0, 1.0, 1.0]
...
def _get_spiral_data(start_x, start_y):
    return [
        {"motor2": start_y + 0.000, "det": 1.0, "motor1": start_x + 0.100},
...
def test_spiral_square(RE, hw):
    ...
    plan = bp.spiral_square(
        [det], motor1, motor2, x_center=0, y_center=0, x_range=3, y_range=4, x_num=3 * 2 + 1, y_num=4 * 2 + 1
    )

    approx_multi_traj_checker(RE, plan, square_spiral_data, decimal=2)
...
def test_rel_spiral_square(RE, hw):
    ...
    plan = bp.rel_spiral_square([det], motor1, motor2, x_range=3, y_range=4, x_num=3 * 2 + 1, y_num=4 * 2 + 1)
