...
class StateMachine(type):
    ...
    def _check_states_enum(cls):
        ...
        try:
            cls.context["states_enum"] = getattr(cls.context.new_class, states_enum_name)
        except AttributeError as err:
            raise ValueError("No states enum given!") from err
    ...
    def _check_state_value(cls):
        ...
        state_value = cls.context.new_meta["translator"].translate(state_value)
    ...
    def _add_standard_attributes(cls):
        ...
        """
        setattr(cls.context.new_class, cls.context.new_meta["state_attribute_name"], cls.context.state_value)
        setattr(cls.context.new_class, cls.context.state_name, utils.state_property)

        cls.context.new_class.is_ = utils.is_
        cls.context.new_class.can_be_ = utils.can_be_
        cls.context.new_class.set_ = utils.set_
    ...
    def _generate_named_checkers(cls):
        ...
        for method, key in named_checkers:
            if method in cls.context.new_methods:
                raise ValueError(
                    f"Name collision for named checker '{method}' - this "
    ...
    def _generate_named_transitions(cls):
        named_transitions = cls.context.get_config("named_transitions", None) or []
        ...
        for item in named_transitions:
            ...
            if method in cls.context.new_methods:
                raise ValueError(
                    f"Name collision for transition '{method}' - this name "
