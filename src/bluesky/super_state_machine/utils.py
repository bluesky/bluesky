...
def set_(self, state):
    ...
    if not self.can_be_(state):
        ...
        raise TransitionError(f"Cannot transit from '{self.actual_state.value}' to '{state.value}'.")
...
class EnumValueTranslator:
    ...
    def generate_search_table(self):
        self.search_table = {item.value: item for item in list(self.base_enum)}
    ...
    def translate(self, value):
        ...
        try:
            ...
            raise ValueError(f"Value {value} doesn't match any state.") from err

    def _check_if_already_proper(self, value):
        if isinstance(value, Enum):
            if value in self.base_enum:
                return True
            raise ValueError(f"Given value ('{value}') doesn't belong to states enum.")
        return False
