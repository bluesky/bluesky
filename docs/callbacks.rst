...
Qt5 equivelant example where generated documents printed as string in a QLabel as well
as the console.
...
In the most recent versions of BlueSky the kickers were deprecated because the
RunEngine now contains a `during_task` parameter that defaults to
`DefaultDuringTask <https://nsls-ii.github.io/bluesky/generated/bluesky.utils.DefaultDuringTask.html?highlight=defaultduringtask#bluesky-utils-defaultduringtask>`_
...
in a Qt application you need to call `install_remote_qt_kicker` and
...
In order to transmit the Bluesky documents over a network, they must be
converted from Python objects to bytes. There are multiple ways to do this:

- pickle: not secure, Python-specific, compact and fast
- JSON: secure, effiectively universal cross-language support, can be large and slow
- msgpack: secure, good cross-lanuage support, compact and fact

The current default, pickle, is no longer recommended because of security
risks: if a consumer (RemoteDispatcher) is handed maliciously-formed bytes, it
may be coerced to execute arbitrary Python code. The default may be changed in
a future release of Bluesky.

This is an example using JSON.

.. code-block:: python

   def safe_json_dump(content):
       """
       A JSON encoder extended to handle some additional types
       """
       import orjson
   ...
   publisher = Publisher('localhost:5577', serializer=safe_json_dump)
   d = RemoteDispatcher('localhost:5578', deseriliazer=orjson.loads)
