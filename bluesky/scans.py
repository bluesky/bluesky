...
class ScanBase(Struct):
    ...
    - an ``_objects`` attribute, a list of all objects the scan can read or set
    ...
    def md(self):
        ...
        # The 'scan_args' key is a dict of kwargs that recreate the object.
        # It is an intentionally redundant subset of the top-level metadata.
        scan_args = {}
        for field in self._fields:
            scan_args[field] = repr(getattr(self, field))
        self._md['scan_args'] = scan_args
        return self._md

    def __iter__(self):
        yield from self._pre_scan()
        # Some metadata is compute in _pre_scan, so it must be done before
...
class Scan1D(ScanBase):
    ...
    def _pre_scan(self):
        self._objects = list(self.detectors) + [self.motor]
        self.num = len(self._abs_steps)
        yield from super()._pre_scan()
...
class ScanND(ScanBase):
    ...
    def _pre_scan(self):
        ...
        self._objects = list(self.detectors) + list(self.motors)
        yield from super()._pre_scan()
...
class _OuterProductScanBase(ScanND):
    ...
    def __init__(self, detectors, *args):
        ...
        self.num = np.product(self.shape)
