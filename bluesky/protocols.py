...
class Movable(Protocol):
    ...
    def set(self, *args, timeout: Optional[float] = None, **kwargs) -> Status:
        """Return a ``Status`` that is marked done when the device is done moving.
        
        ``args`` and ``kargs`` are device specific, but it timeout is specified
        then the move should raise an exception of the move takes longer than this.
        """
        ...
...
class Hints(TypedDict, total=False):
    ...
    #: Include this if scan data is scampled on a regular rectangular grid
