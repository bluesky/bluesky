...
class Publisher:
    ...
    address : string or tuple
        Address of a running Kafka server, given either as a string like
        ``'127.0.0.1:9092'`` or as a tuple like ``('127.0.0.1', 9092)``
    topic : string, optional
        Kafka topic string.
    pykafka : object, optional
        By default, the 'pykafka' module is imported and used. Anything else
    ...
    """
    def __init__(self, address, *, topic_name, pykafka=None, serializer=pickle.dumps):
        if pykafka is None:
            import pykafka
        self.address = address
        self.topic_name = topic_name
        self.kafka_client = pykafka.KafkaClient(hosts=self.address)
        self._serializer = serializer

    def __call__(self, name, doc):
        doc = copy.deepcopy(doc)
        message = b' '.join([name.encode(),
