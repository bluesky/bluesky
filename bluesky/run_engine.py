...
class RunEngine:
    ...
    plan_traceback_handler : callable, optional
        A function that inspects the currently handled exception and returns
        a list of strings to display in IPython instead of the normal
        traceback render. This can be used to add context or remove spam as
        appropriate to be more user-friendly.
    ...
    def __init__(self, md=None, *, loop=None, preprocessors=None,
        ...
        self._plan_traceback_handler = plan_traceback_handler  # Returns list[str] as the traceback
            ...
            try:
                ...
                if self._task_fut.done():
                    ...
                    if (exc is not None
                        ...
                        if self._plan_traceback_handler is not None:
                            # Custom traceback in ipython
                            # Can remove spam and add additional context
                            try:
                                orig_render = exc._render_traceback_
                            except AttributeError:
                                exc._render_traceback_ = self._plan_traceback_handler
                            else:
                                # Case where the exception already had a renderer
                                def include_orig_render():
                                    lines = orig_render()
                                    lines.extend(self._plan_traceback_handler())
                                    return lines
                                exc._render_traceback_ = include_orig_render
                        raise exc
