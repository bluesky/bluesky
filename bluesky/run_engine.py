...
class RunEngine:
    ...
    call_returns_uid: bool, optional, default True
        A flag that controls the return value of __call__. If True, calling the
        RunEngine will return a list of uids. If False, RunEngine will pass through
        the return value of the plan that was run.
    ...
    def __init__(self, md=None, *, loop=None, preprocessors=None,
                 ...
                 during_task=None, call_returns_uid=True):
        if loop is None:
        ...
        self._call_returns_uid = call_returns_uid # should __call__ return UID or plan value
    ...
    def __call__(self, *args, **metadata_kw):
        ...
        Returns
        -------
        if instantiated with call_returns_uid = True
        uids : list
            list of uids (i.e. RunStart Document uids) of run(s)

        if instantiated with call_returns_uid = False
        returns the value returned by plan
        ...
        plan_result = self._resume_task(init_func=_build_task)

        if self._interrupted:
            raise RunEngineInterrupted(self.pause_msg) from None

        if self._call_returns_uid:
