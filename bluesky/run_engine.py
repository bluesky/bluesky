...
class RunEngine:
    ...
    def __init__(self, md=None, *, loop=None, preprocessors=None,
        ...
        self._bundle_type = None  # type of bundle (partial, other)
        ...
        self._describe_cache_dict = defaultdict(dict)
    ...
    def _clear_run_cache(self):
        ...
        self._describe_cache_dict.clear()
    ...
    def _create(self, msg):
        ...
        self._bundle_type = None  #  unknown until we read
    ...
    def _read(self, msg):
        ...
        """
        obj = msg.obj
        # actually _read_ the object if streaming this will be passed here
        # for backwards compatibility, only pass the streaming kwarg if the
        # object can take streaming as an argument (else don't read)
        if msg.kwargs.pop('streaming', False):
            if 'streaming' in inspect.signature(obj.read).parameters:
                ...
                if hasattr(obj, 'collect_asset_docs'):
                    datum_docs = list(obj.collect_asset_docs(*msg.args,
                                                             **msg.kwargs))
                else:
                    datum_docs = []
            else:
                return {}
            bundle_type = 'streaming'
        else:
        ...
        if self._bundle_type is None:
            self._bundle_type = bundle_type
        elif self._bundle_type != bundle_type:
            raise ValueError("Error, trying to read different"
                             "Types of readings in a stream"
                             "Got {}, but already of type {}"\
                             .format(bundle_type, self._bundle_type))

        if self._bundling:
            # just cache them, we only emit descriptors on save()
            desc_key = self._bundle_name
            # it's a defaultdict(dict) so if it doesn't exist, will return
            # a reference to a dict that the object will point to
            describe_cache = self._describe_cache_dict[desc_key]
            ...
            if obj not in describe_cache:
                ...
                if self._bundle_type =='streaming':
                    if 'streaming' in inspect.signature(obj.describe).parameters:
            ...
            # configuration should be same whether streaming or not
            ...
            # current event
            cur_keys = set(describe_cache[obj].keys())
            for read_obj in self._objs_read:
                # that is, field names
                known_keys = describe_cache[read_obj].keys()
    ...
    def _save(self, msg):
        ...
        if doc is None:
            ...
            for obj in objs_read:
                dks = self._describe_cache_dict[desc_key][obj]
                name = obj.name
